-- Creates an Outlook draft using the `plain text content` property, which
-- preserves line breaks exactly as written. If this build of Outlook does not
-- expose that term the script fails to compile and the app falls back to
-- outlook_draft.applescript.
--
-- Run: osascript outlook_draft_plain.applescript <toAddress> <toName> <subject> <body> <attachmentPath>
